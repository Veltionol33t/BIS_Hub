...
export default function Hub(){
 ...
 const downloadCsv=(name:string,rows:Tx[],raw=false)=>{const data=raw?rows:exportRows(rows);if(!data.length){alert('Nothing to export.');return}const keys=Object.keys(data[0]);const esc=(v:any)=>`"${String(v??'').replace(/"/g,'""')}"`;const csv=[keys.join(','),...data.map(r=>keys.map(k=>esc((r as any)[k])).join(','))].join('\n');const blob=new Blob([csv],{type:'text/csv;charset=utf-8;'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=name;a.click();URL.revokeObjectURL(a.href)};
 ...
 return <div className="shell">
  ...
  <main className="main">
   ...
   {page==='dashboard'&&<>
    ...
    {workspace==='whole'&&companyButtons.length>0&&<section className="company-strip"><div className="section-kicker">Separate company workspaces</div><div className="company-grid">{companyButtons.map(c=>{const x=companySummaries[c.id]||{};const rows=tx.filter(t=>t.business_unit_id===c.id).slice(0,5);return <section className="company-card company-section" key={c.id}><div className="company-card-head"><div><span>{c.code||'COMPANY'}</span><b>{c.name}</b></div><button className="btn small primary" onClick={()=>setWorkspace(c.id)}>Open</button></div><div className="company-metrics"><div><small>Income</small><strong>{money(x.income)}</strong></div><div><small>Expenses</small><strong>{money(x.expenses)}</strong></div><div><small>Net VAT</small><strong>{money(x.net_vat)}</strong></div></div><div className="company-recent"><div className="company-recent-title">Recent activity</div>{rows.length?rows.map(t=><div className="company-tx" key={t.id}><span>{t.transaction_date}</span><span>{t.counterparty||t.description||'Transaction'}</span><b className={t.type==='expense'?'expense-text':'income-text'}>{t.type==='expense'?'-':'+'}{money(t.amount)}</b></div>):<div className="company-none">No transactions yet.</div>}</div></section>})}</div></section>}
    <section className="panel">
     ...
     <Table rows={tx.slice(0,10)} onEdit={openTx} onDelete={deleteTx} businessUnits={bus}/>{!tx.length&&<div className="empty">No transactions yet.</div>}
    </section>
   ...
   {page==='reports'&&<section className="panel"><div className="panel-head"><div><div className="panel-title">Monthly tally · {title}</div><div className="sub">Switch Workspace above to view Bringing Industry Solution, Bethesher, Whole, or Personal / Other.</div></div><button className="btn primary" onClick={()=>downloadCsv(`monthly-tally-${workspace}.csv`,summary.monthly||[],true)}>Export CSV</button></div><MonthlyTable rows={summary.monthly||[]}/></section>}
...
function Table({rows,onEdit,onDelete,businessUnits=[]}:{rows:Tx[];onEdit:(x:Tx)=>void;onDelete:(id:string)=>void;businessUnits?:BU[]}){return <div className="table-wrap"><table className="table transaction-table"><thead><tr><th>Date</th><th>Type</th><th>Customer / Supplier</th><th>Category</th><th>Account</th><th>Company</th><th>Reference</th><th>VAT</th><th className="right">Amount</th><th></th></tr></thead><tbody>{rows.map(x=><tr key={x.id}><td>{x.transaction_date}</td><td><span className={'pill '+x.type}>{x.type}</span></td><td>{x.counterparty||'—'}</td><td>{x.categories?.name||'—'}</td><td>{x.accounts?.name||'—'}</td><td>{businessUnits.find(b=>b.id===x.business_unit_id)?.name||'—'}</td><td>{x.reference_number||'—'}</td><td>{x.tax_type==='vat'?money(x.tax_amount):'—'}</td><td className="right">{money(x.amount)}</td><td><button className="btn small" onClick={()=>onEdit(x)}>Edit</button> <button className="btn small danger" onClick={()=>onDelete(x.id)}>Delete</button></td></tr>)}</tbody></table></div>}
